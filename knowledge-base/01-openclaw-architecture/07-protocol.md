...
> 协议是 OpenClaw 客户端与服务端之间的"契约层":所有跨进程通信都走自研 WebSocket 协议,由 TypeBox Schema 严格定义。读完本章你将理解协议版本如何演进、14 个 Schema 分片如何组合、为什么版本 bump 是重大决策。

## 一句话定位

Gateway 协议是客户端与服务端之间的**唯一契约**:
- 自研 WebSocket 协议,用 TypeBox Schema 定义所有消息格式(非 freeform string)
- 14 个 Schema 分片按领域拆分(传输 / 会话 / 调度 / 插件 / 通道 / 审批等),组合器拼成完整协议
...
| **Schema 组合器** | 把 14 个分片拼成完整协议 schema | 分片按领域拆分,避免单文件巨型 schema |
| **14 个 Schema 分片** | 按领域定义消息格式(传输 / 会话 / 调度 / 插件等) | 用 TypeBox,非 freeform string |
...
- **怎么做**:14 个 fragment 按领域拆分(传输 / 会话 / 调度 / 插件 / 通道 / 审批等);组合器拼成完整协议;40+ 领域 schema 独立维护。
