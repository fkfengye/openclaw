...
> 读完本章你将理解:10 个厂商适配器各自的职责与特点,为什么 Cloudflare 只做元数据,Copilot 只做头部注入,以及各适配器如何共用传输层而不重复网络策略。
...
下图展示 10 个适配器如何挂载到 API 注册表,以及它们与传输层的分工。
...
└────┬─────┘└────┬─────┘└────┬─────┘└────┬─────┘└────┬─────┘
     ...
     ▼           ▼           ▼           ▼           ▼
┌──────────┐┌──────────┐┌──────────┐┌──────────┐┌──────────┐
│  Vertex  ││ Mistral  ││Cloudflare││ Copilot  ││ OpenAI   │
│ Google企 ││ 对话流   ││ 元数据   ││ 头部注入 ││ ChatGPT  │
│          ││          ││          ││          ││ 响应流   │
│•企业认证 ││•对话翻译 ││•URL 占位 ││•发起者推 ││•复用     │
│•共享翻译 ││•有界流   ││  符替换  ││  断      ││  OpenAI  │
│          ││          ││•厂商判断 ││•视觉头部 ││  响应流  │
│          ││          ││•不直接   ││•意图头部 ││•ChatGPT  │
│          ││          ││  fetch   ││          ││  会话契约│
└────┬─────┘└────┬─────┘└──────────┘└──────────┘└──────────┘
...
10 个适配器分三类:完整流式适配器、共享翻译适配器、辅助适配器。
...
| **OpenAI ChatGPT 响应流** | 翻译 OpenAI ChatGPT 响应 API(基于 Codex harness) | 复用 OpenAI 响应流;ChatGPT 专属会话契约 |
...
| 02-provider-implementations.md | 本文件 — Provider 实现:10 个厂商适配器 |
